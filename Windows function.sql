...
select id , dept , salary , sum(salary) over() , avg (salary) over() from employees;

-- windows function for each column because we cant use aggregate function  so we use windows functin 
-- taking sum of every department differently

select id , dept , salary , sum( salary) over(partition by dept) from employees;

-- rolling sum : sum of column to next column 
 
 select id , dept , salary , sum(salary) over (partition by dept ) , sum(salary) over (partition by dept order by salary )  from employees;
 
 -- rank function  : if the data is same it will give same rank 1 for 2 data and for new data it will start  from 3
 -- dense rank : if the data is same it will not  give same rank 1 for 2 data as 1 ,2  and for new data it will start  from 3
 
 select id , dept , salary , row_number() over() , rank() over (partition by dept order by salary desc ) from employees;
 select id , dept , salary , row_number() over() , dense_rank() over (partition by dept order by salary desc ) from employees;
